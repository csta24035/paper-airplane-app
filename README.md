...
Airplane と Instruction の間には "1" *-- "0..50"（1つの紙飛行機に対して折り方は最大50件）を定義。
...
画面(UI): AirplaneForm や DetailPage など、ユーザー入力を受け付け、表示を担当。
...
登録フローでは「登録件数が100件に達しているかどうか」による全体分岐と、その内部での「IndexedDBの容量超過により保存に失敗したか」のネストされた分岐を実装しています。
